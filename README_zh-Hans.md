...
普通成员举报时，bot 只保存证据，目标消息和举报消息都保留在群内。群主或管理员举报时，bot 会永久封禁超级群中的非管理员发送者，阻止其评论和再次入群，直到管理员解除封禁。bot 单独删除被举报的消息，并通过 Telegram 的封禁接口请求清理发送者的历史消息。bot 无法独立确认历史清理结果；管理员应检查是否有残留消息，必要时手动删除。举报前应核对回复目标：解除封禁无法恢复已删除的消息。

Telegram 确认目标消息已删除或不存在后，bot 才会清理管理员的举报消息。如果 Telegram 拒绝删除目标，bot 会保留举报；封禁成功时，日志会记录 `deletion rejected; banned`。管理员应手动删除残留消息。匿名管理员可以用群组本身的身份举报；bot 不处理以关联频道或其他聊天身份发送的举报。
...
接着在群管理的成员权限设置中解除该账号的禁言，再发送一条测试消息或贴纸。先用普通成员账号举报，确认消息保留；再用管理员账号举报，确认目标消息和举报消息都已删除，发送者无法通过关联频道继续评论，也无法通过邀请链接重新入群。单独检查 Telegram 是否清理了该账号的历史消息。其他用户的消息应保留。
