...
| 用户通过被拦截的来源 bot 发消息 | 删除当前消息，永久禁言非管理员发送者，保留其他历史消息和成员身份。封禁来源 bot，清理该 bot 符合条件的索引消息。 |
| 账号黑名单中的用户发消息 | 对尚未封禁的账号执行封禁，清理符合条件的索引消息。 |

群主和管理员免于禁言、封禁。来源和账号黑名单匹配也不会删除其消息；获授权管理员举报仍可删除指定消息。

用户 A 通过来源 bot B 发消息时，`blacklisted_sources` 保存的是 B。来源过滤不会将 A 加入任何黑名单。B 确认封禁成功后也会进入 `blacklisted_users`，复用账号黑名单的封禁和索引历史清理流程。
...
命令添加来源，不改变成员状态、清理历史或写入账号黑名单。群聊中，bot 回复结果后会删除命令消息，用户名无法解析时也会清理；私聊中的命令和结果回复会保留。后续消息中的内联 bot（`via_bot.id`）或转发来源 bot 命中名单时，按上表中的来源过滤策略处理。
