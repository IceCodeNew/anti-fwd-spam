...
在超级群中，管理员或已授权的群组、频道身份举报后，bot 删除目标消息、封禁发送者，并清理符合条件的索引历史。真人账号和 bot 账号适用相同规则，群主和管理员免于封禁。清理成功后，bot 删除举报消息。已授权普通成员的举报只保存证据，不删除消息或限制成员。

如果 Telegram 拒绝封禁（例如 bot 没有封禁权限），bot 仍会删除目标消息。在论坛话题中，需要回复垃圾消息本身；没有回复任何消息的提及不会举报话题创建者。未授权身份发出的举报和 `/bs` 命令，bot 按普通消息审核。这类举报被本地规则或 Jev 删除时，如果被举报的消息本身命中本地规则，bot 不禁言举报人。

确认封禁后，发送者进入账号黑名单。该账号在使用同一 bot 的其他超级群发言时，bot 会在该群封禁账号并清理符合条件的索引消息。

如果消息由 A 通过 bot B 代发，这种回复举报只针对 A，不登记或处罚 B。

### 添加来源

用已授权身份在私聊或群聊中发送 `/bs @example_bot`，将 `example_bot` 替换为来源 bot 的用户名。群内可以使用 `/bs@管理bot用户名 @example_bot` 明确指定接收命令的 bot。

bot 将解析出的账号 ID 保存到来源名单，并回复该 ID。群聊中，bot 回复后会删除命令消息，查询失败时也会清理；私聊命令和结果回复会保留。这个指定用户名的命令不封禁账号或删除历史。请核对返回的 ID：Telegram 无法解析部分用户名，命令也不校验目标是否为 bot。

同时举报代发消息的发送者 A 和来源 bot B 时，回复该消息并发送 `/bs`（或 `/bs@管理bot用户名`），无需填写用户名。管理 bot 从消息中读取 B 的 ID 并加入来源名单。获授权管理员的举报会封禁 A 和 B，分别清理其符合条件的索引历史，保留管理员保护。已授权普通成员可以登记 B，但不能触发这些处罚。处理结束后，bot 清理群内命令；临时失败时保留命令，等待重试。

无法确认封禁结果时，bot 保留命令，并提示举报者检查成员状态后再提交新举报，不会自动重复未确认的封禁。
...
群主和管理员免于禁言、封禁，来源 bot 是管理员时也受保护。自动过滤不会删除其消息。获授权管理员举报仍可删除指定的管理员消息。
...
bot 在 Jev 之前用本地规则检查群内新消息和编辑后的消息，包括其他 bot 的消息，无需配置 API key。规则在正文或媒体说明的任意位置搜索 campaign 标记，以及连续相同的钱袋或红圆 emoji，允许 emoji 之间有空白。前后文字或其他 emoji 不影响匹配。具体匹配式见 [policy.py](src/anti_fwd_spam/policy.py) 中的 `SPAM_PATTERNS`。被回复的消息内容和单个敏感词不触发这些规则。
...
bot 在其他管理规则之后，用 Jev 检查用户新消息。垃圾消息分数达到阈值时，bot 删除该条消息并永久禁言发送者，保留其历史消息，不加入任何黑名单。群主和管理员受保护。阈值见 [model.py](src/anti_fwd_spam/model.py) 中的 `SPAM_THRESHOLD`；分数不代表准确率。编辑消息、服务事件、bot 消息及以群组或频道身份发送的消息不参与检查。
...
D1 保留举报和来源处理的完整 JSON、命令解析出的 ID，以及临时处理记录，保留期为 3 天。近期消息索引只保存 ID 和时间，不保存正文。模型任务在判断完成前暂存提交的内容，之后只保留完成删除和禁言所需的身份信息。bot 不下载媒体文件。
