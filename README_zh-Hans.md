...
普通成员举报时，bot 只保存证据，目标消息和举报消息都保留在群内。群主或管理员举报时，bot 会永久封禁超级群中的非管理员发送者。Telegram 将该用户移出群组，阻止其再次加入，直到管理员解除封禁，并删除该用户在群内的全部历史消息。举报前应核对回复目标：解除封禁无法恢复已删除的消息。

bot 也会清理已处理的管理员举报消息。管理员应检查目标用户的成员状态，不能仅凭举报消息消失判断封禁是否完成。匿名管理员可以用群组本身的身份举报；bot 不处理以关联频道或其他聊天身份发送的举报。

目标处理结果确认后，重试只会清理举报消息。如果日志出现 `ban confirmation failed`，表示 bot 无法确认结果，不会因同一条举报再次封禁。管理员应检查发送者的成员状态；如果仍需处理，可以发送新的举报，或在 Telegram 的成员设置中手动封禁。bot 会保留未确认的举报消息，供管理员检查。
...
接着在群管理的成员权限设置中解除该账号的禁言，再发送一条测试消息。先用普通成员账号举报，确认消息保留；再用管理员账号举报，确认发送者被移出群组、无法通过邀请链接重新加入，并且该账号在测试群中的全部历史消息都已删除。其他用户的消息应保留。

启用「保持匿名」（Remain Anonymous），以群组本身的身份再次执行管理员举报，确认封禁和历史消息删除仍然生效。
