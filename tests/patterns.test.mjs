...
for (const text of [campaign, flood, '@example_bot campaign_2', '💰'.repeat(10), '🔴 '.repeat(10)]) {
...
test('user: Given ordinary discussion and short emoji runs, When Jev accepts them, Then local filtering leaves them visible', async () => {
  const examples = [
    ...
    `请警惕这种垃圾消息：${campaign}`, `示例：${'💰'.repeat(10)}`,
    '@example_bot campaign_update', '@example_bot campaign_1a',
    '@example_bot campaign_1 this is a discussion', '💰'.repeat(9), '🔴🔴',
    '💰'.repeat(5) + '🔴'.repeat(5), '普通消息',
  ];
